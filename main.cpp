...
BOOL DoFileOpenSave(HWND hwnd, BOOL bSave) {
	...
	ofn.hwndOwner = hwnd;
	ofn.lpstrFilter = "Java Files (*.java)\0*.java\0All Files (*.*)\0*.*\0\0";
	ofn.lpstrFile = szFileName;
	ofn.nMaxFile = MAX_PATH;
	ofn.lpstrDefExt = "java";
...
void CompileJavaFiles(HWND hwnd){
	/*string cmd="javac ";
	Py_Initialize();
	FILE* fp;
	fp=_Py_fopen("py_input.py","r");
	PyRun_SimpleFile(fp,"py_input.py");
	Py_Finalize();
	string str;
	freopen("cmd_input.txt","r",stdin);
	cin>>str;
	cmd+=str;
	char* ch;
	ch=cmd.c_str();*/
		...
		case WM_COMMAND:
			switch(LOWORD(wParam)) {
				...
				case CM_FILE_EXIT:
					...
					break;
				case CM_COMPILE:
					CompileJavaFiles(hwnd);
					break;
				case CM_ABOUT:
					MessageBox (NULL, "Java Editor for Windows!\nCreated using the Win32 API.\nMade by Lcj.\n2022/12/23" , "About", 0);
					break;
			}
			break;
		case WM_CLOSE:
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpCmdLine, int nCmdShow) {
	...
	hwnd = CreateWindowEx(WS_EX_CLIENTEDGE,"WindowClass","Java Editor",WS_OVERLAPPEDWINDOW,
		CW_USEDEFAULT,
		CW_USEDEFAULT,
		480,320,
